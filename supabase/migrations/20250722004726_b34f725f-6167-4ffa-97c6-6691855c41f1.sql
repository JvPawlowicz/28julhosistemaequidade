...
CREATE TABLE IF NOT EXISTS public.evolutions (
  ...
  status TEXT NOT NULL DEFAULT 'Rascunho', -- Rascunho, Pendente de Supervisão, Finalizada
...
-- Remover temporariamente policies que usam status
DROP POLICY IF EXISTS "Therapists can manage own evolutions" ON public.evolutions;
DROP POLICY IF EXISTS "Coordinators can update supervision status" ON public.evolutions;

-- Alterar coluna status para usar o enum
ALTER TABLE public.evolutions ALTER COLUMN status TYPE evolution_status USING status::evolution_status;

-- Recriar policies removidas
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Therapists can manage own evolutions' AND tablename = 'evolutions') THEN
    CREATE POLICY "Therapists can manage own evolutions" 
    ON public.evolutions 
